    ...
    toast:function({title, message, type, location, icon, theme, customIcon, dismissable, onClick, onRender, onTimeout, customHTML, primaryButton, secondaryButton}){
        ...
        if(document.getElementById('toaster') == null){
            ...
            if(document.getElementById('butterupRack') == null){
                ...
                rack.className = 'rack';
                // check if the user has stacked toasts enabled
                if(butterup.options.stackedToasts == true){
                  // check if the location is top-right, top-center, or top-left
                  if(location == 'top-left' || location == 'top-center' || location == 'top-right' || location == null){
                    // we need to add the upperstack class to the rack
                    rack.classList.add('upperstack');
                  }
                  // check if the location is bottom-right, bottom-center, or bottom-left
                  if(location == 'bottom-left' || location == 'bottom-center' || location == 'bottom-right'){
                    // we need to add the lowerstack class to the rack
                    rack.classList.add('lowerstack');
                  }
                }
